...
namespace OrderManage
{
    ...
    class Order : IComparable
    {
        ...
        {
            if (this.customer != order.customer) // 顾客不同不可能是相同订单
            {
                return false;
            }
            if (this.details.itemList.Count != order.details.itemList.Count) // 商品种类数不同不可能是相同订单
            {
                return false;
            }

            int sameCount = 0;
            foreach (Item item1 in this.details.itemList)
            {
                foreach (Item item2 in order.details.itemList)
                {
                    // 如果找到了一个一样的商品，相同商品数量++
                    if (item1.itemName == item2.itemName && item1.itemNum == item2.itemNum)
                    {
                        sameCount++;
                    }
                }
            ...
            // 所有商品都相同
            if (sameCount == this.details.itemList.Count)
            {
                return true;
            }
            return false;
        ...
        {
            string str = "订单编号:" + this.orderId + "\t" + "客户:" + this.customer + "\n";
            foreach (Item item in this.details.itemList)
            {
                str += (item.itemName + ":" + item.itemNum + "件\t\n");
            }
            str += "\n";
    ...
    class OrderService
    {
        ...
        // 由于没有金额，所以按照订单的商品种类数排序
        public void sortByItemKinds()
        {
            ArrayList<Order> list = this.orderList.Sort((a, b) => a.details.itemList.Count - b.details.itemList.Count);
